...
const startReconnecting = (event: MouseEvent, conn: Connection) => {
  event.stopPropagation()
...
const finishConnecting = (toNodeId: string) => {
  if (activeDrawing.value && activeDrawing.value.fromId !== toNodeId) {
    const exists = connections.value.some(c => c.fromId === activeDrawing.value?.fromId && c.toId === toNodeId)
    if (!exists) {
...
const onGlobalMouseUp = () => {
  ...
  if (!event?.target?.closest('.node-input-port')) activeDrawing.value = null
}
  ...
  <BaseLayout>
    <div class="flex w-full h-full overflow-hidden relative select-none">
      ...
      <main ref="canvasRef" @dragover.prevent @drop="onCanvasDrop" class="flex-1 relative bg-[#0F1928] overflow-hidden" style="background-image: radial-gradient(circle, rgba(255, 255, 255, 0.03) 1px, transparent 1px); background-size: 30px 30px;">
        <!-- Nodes -->
        <div v-for="node in nodes" :key="node.id" class="absolute p-4 bg-[#1A2536]/90 backdrop-blur-xl border rounded-xl shadow-2xl transition-all" :style="{ left: node.x+'px', top: node.y+'px', width: NODE_WIDTH+'px', height: NODE_HEIGHT+'px', borderColor: isDraggingNode === node.id ? node.color : 'rgba(255,255,255,0.1)', zIndex: 10 }" @mousedown="startDragNode($event, node.id)" @contextmenu="openContextMenu($event, 'node', node.id)">
          <div class="absolute -left-2 top-1/2 -translate-y-1/2 w-4 h-4 bg-[#0F1928] border-2 border-white/20 rounded-full z-20 cursor-crosshair node-input-port hover:border-[#3B9BFF] hover:scale-125 transition-all" @mouseup="finishConnecting(node.id)"></div>
        ...
        <!-- SVG Layer (Now properly handling context menu) -->
        <svg class="absolute inset-0 w-full h-full pointer-events-none z-[30]">
          ...
          <g v-for="conn in connections" :key="conn.id" class="pointer-events-auto cursor-pointer group" @contextmenu="openContextMenu($event, 'conn', conn.id)">
            ...
            <circle :cx="nodes.find(n => n.id === conn.toId)?.x || 0" :cy="(nodes.find(n => n.id === conn.toId)?.y || 0) + NODE_HEIGHT / 2" r="6" fill="#3B9BFF" class="opacity-0 group-hover:opacity-100 cursor-move" @mousedown="startReconnecting($event, conn)" />
          </g>
        ...
        <div v-if="contextMenu.show" class="fixed z-[100] bg-[#1A2536] border border-white/10 rounded-lg shadow-2xl py-1 min-w-[120px]" :style="{ left: contextMenu.x + 'px', top: contextMenu.y + 'px' }" @click.stop>
          <button @click="handleDelete" class="w-full text-left px-4 py-2 text-sm text-red-400 hover:bg-red-500/10 flex items-center gap-3"><iconify-icon icon="lucide:trash-2"></iconify-icon>删除{{ contextMenu.type === 'node' ? '节点' : '线条' }}</button>
        </div>
      </main>
