        ...
        <div v-if="isCreateModalOpen" class="fixed inset-0 z-[120] flex items-center justify-center p-6">
          ...
          <div class="relative w-full max-w-lg bg-[#1A2536] border border-white/10 rounded-3xl shadow-2xl overflow-hidden flex flex-col">
            <div class="p-6 border-b border-white/10 flex justify-between items-center bg-white/5">
              <h2 class="text-xl font-bold text-white/95">添加新模型</h2>
            ...
            <div class="p-8 space-y-4">
              <input v-model="newModel.name" placeholder="模型显示名称" class="w-full bg-black/20 border border-white/10 rounded-xl px-4 py-3 text-sm text-white outline-none">
              <input v-model="newModel.base_url" placeholder="API Base URL (e.g., https://api.openai.com/v1)" class="w-full bg-black/20 border border-white/10 rounded-xl px-4 py-3 text-sm text-white outline-none">
              <input v-model="newModel.model_name" placeholder="模型标识 (e.g. gpt-4)" class="w-full bg-black/20 border border-white/10 rounded-xl px-4 py-3 text-sm text-white outline-none">
              <input v-model="newModel.api_key" placeholder="API Key" type="password" class="w-full bg-black/20 border border-white/10 rounded-xl px-4 py-3 text-sm text-white outline-none">
              <textarea v-model="newModel.description" placeholder="模型描述..." class="w-full bg-black/20 border border-white/10 rounded-xl px-4 py-3 text-sm text-white outline-none h-24"></textarea>
            </div>
            <div class="p-6 border-t border-white/10 bg-white/5 flex gap-3">
              ...
              <button @click="handleCreate" class="flex-1 py-3 rounded-xl bg-[#3B9BFF] text-white text-sm font-bold shadow-[0_0_20px_rgba(59,155,255,0.4)]">保存模型</button>
            </div>
          </div>
        </div>
      </Transition>
